...
public interface NewsService {
	...
	@Select("SELECT * FROM tysccj_test.NEWS WHERE ID = #{id};")
	News findById(News news);
	
	@Select({
		"<script>",
		"SELECT N.*,C.NAME AS CATEGORYNAME,C.IMAGE AS CATEGORYIMAGE FROM tysccj_test.NEWS N ",
		"LEFT JOIN tysccj_test.NEWS_CATEGORY C ON N.CATEGORY = C.ID ",
		"WHERE N.STATE = 1 ",
	...
	@Select({
		"<script>",
		"SELECT COUNT(*) FROM tysccj_test.NEWS N ",
		"LEFT JOIN tysccj_test.NEWS_CATEGORY C ON N.CATEGORY = C.ID ",
		"WHERE N.STATE = 1 ",
			"<when test='title!=null'>",
				"AND N.TITLE LIKE CONCAT('%',#{title},'%')",
	...
	@Insert("INSERT INTO `tysccj_test`.`news` (`id`,`title`,`description`,`category`,`image`,`content`,`addDate`,`updateDate`,`commendState`,`state`,`browses`,`likes`,`comments`,`score`) VALUES (null,#{title},#{description},#{category},#{image},#{content},now(),now(),1,1,0,0,0,0);")
	int insert(News news);

	@Update("UPDATE `tysccj_test`.`news` SET `title` = #{title}, `description` = #{description}, `category` = #{category}, `image` = #{image}, `content` = #{content}, `updateDate` = now()  WHERE `id` = #{id};")
	int update(News news);
	
	@Update("UPDATE `tysccj_test`.`news` SET `state` = #{state}, `commendState` = #{commendState}, `browses` = #{browses}, `likes` = #{likes}, `comments` = #{comments}, `score` = #{score} WHERE `id` = #{id};")
